...
pivot_df = (
    windowed_df.groupBy("window", "region", "song_id")
...
# Compute skip ratio
skip_ratio_df = pivot_df.withColumn(
    "skip_ratio", col("skip") / (col("play") + col("skip"))
)

# Rank songs by highest skip ratio per region + time window
w_skip = Window.partitionBy("region", "window").orderBy(desc("skip_ratio"))

ranked_skip_df = skip_ratio_df.withColumn("rn", row_number().over(w_skip)).filter(
    col("rn") <= 5
)
