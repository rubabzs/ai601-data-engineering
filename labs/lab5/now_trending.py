# now_trending.py

from pyspark.sql import SparkSession
from pyspark.sql.functions import col, window, from_json, sum as _sum, when
from pyspark.sql.functions import desc
from pyspark.sql.types import StructType, StructField, StringType, DoubleType
from pyspark.sql.types import TimestampType
from pyspark.sql.window import Window
from pyspark.sql.functions import row_number

# 1) Create SparkSession
spark = SparkSession.builder.appName("NowTrendingSongs").getOrCreate()

spark.sparkContext.setLogLevel("WARN")

# 2) Read from Kafka
kafka_df = (
    spark.readStream.format("kafka")
    .option("kafka.bootstrap.servers", "localhost:9092")
    .option("subscribe", "music_events")
    .option("startingOffsets", "latest")
    .load()
)

# 3) Parse the JSON 'value' from Kafka
schema = StructType(
    [
        StructField("song_id", StringType(), True),
        StructField("timestamp", DoubleType(), True),  # or we can store as Double
        StructField("region", StringType(), True),
        StructField("action", StringType(), True),
    ]
)

json_df = kafka_df.selectExpr("CAST(value AS STRING) as json_str")

parsed_df = json_df.select(from_json(col("json_str"), schema).alias("data"))
events_df = parsed_df.select("data.*")

# Convert timestamp double -> actual timestamp if we want event time
# But for simplicity, let's do a processing-time approach
# If you want event-time windows, do:
# events_df = events_df.withColumn("event_time", (col("timestamp") * 1000).cast(TimestampType()))
from pyspark.sql.functions import current_timestamp

# 4) Filter only "play" events
plays_df = events_df.filter(col("action") == "play")
aggregated_df = events_df.groupBy(
    window(current_timestamp(), "60 second"), col("region"), col("song_id")
).agg(
    _sum(when(col("action") == "play", 1).otherwise(0)).alias("play_count"),
    _sum(when(col("action") == "skip", 1).otherwise(0)).alias("skip_count"),
)
aggregated_df = aggregated_df.withColumn(
    "skip_ratio",
    when(
        (col("play_count") + col("skip_count")) > 0,
        col("skip_count") / (col("play_count") + col("skip_count")),
    ).otherwise(0),
)
unpopular_songs_df = aggregated_df.filter(col("skip_ratio") > 0.5)


# 5) Group by region + 5-minute processing time window
# We'll do a simple processing-time window using current_timestamp
# Alternatively, you can do event-time with a column if you convert 'timestamp' to a Spark timestamp


windowed_df = plays_df.groupBy(
    window(current_timestamp(), "60 second"),  # processing-time window
    col("region"),
    col("song_id"),
).count()


# 6) Use foreachBatch to do rank-based top N logic each micro-batch
def process_batch(batch_df, batch_id):
    """
    This function is called for each micro-batch. We treat 'batch_df' as a normal batch DataFrame.
    We'll rank by 'count' within each region & window and pick top 3 (or 5, or 100).
    """
    if batch_df.rdd.isEmpty():
        print("No data in this batch.")
        return

    # We'll partition by region + the 'window' column
    w = Window.partitionBy("region", "window").orderBy(desc("count"))

    ranked_df = batch_df.withColumn("rn", row_number().over(w)).filter(col("rn") <= 3)

    # Show the top songs for each region + 5-min window
    print(f"=== Batch: {batch_id} ===")
    ranked_df.show(truncate=False)


def process_unpopular_song_batch(batch_df, batch_id):
    if batch_df.rdd.isEmpty():
        print("No data in this batch.")
        return

    # Partition by region and window, then rank by descending skip_ratio (or you could include play_count/skips as needed)
    w = Window.partitionBy("region", "window").orderBy(desc("skip_ratio"))
    ranked_df = batch_df.withColumn("rn", row_number().over(w)).filter(col("rn") <= 3)

    # Display the top songs with the highest skip ratios per region & window
    print(f"=== Batch: {batch_id} ===")
    ranked_df.show(truncate=False)


unpopular_query = (
    unpopular_songs_df.writeStream.outputMode("update")
    .foreachBatch(process_unpopular_song_batch)
    .start()
)
unpopular_query.awaitTermination()

# 7) Write Stream with foreachBatch
query = windowed_df.writeStream.outputMode("update").foreachBatch(process_batch).start()

query.awaitTermination()
